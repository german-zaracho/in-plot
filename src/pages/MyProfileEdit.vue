...
<template>
    ...
    <form action="#" @submit.prevent="handleSubmit">
        <div class="mb-4">
            ...
            <input type="text" id="displayName" class="w-full p-2 border rounded read-only:bg-gray-200" :readonly="editing" v-model="editData.displayName">
        </div>
        <div class="mb-4">
            <label class="block mb-2" for="favMovie">Favorite movie</label>
            <input type="text" id="favMovie" class="w-full p-2 border rounded read-only:bg-gray-200" :readonly="editing" v-model="editData.favMovie">
        </div>
        <div class="mb-4">
            <label class="block mb-2" for="favSeries">Favorite series</label>
            <input type="text" id="favSeries" class="w-full p-2 border rounded read-only:bg-gray-200"  :readonly="editing" v-model="editData.favSeries">
        </div>
        <div class="mb-4">
            <label class="block mb-2" for="anAdditionalComment">Additional comments</label>
            <textarea id="anAdditionalComment" class="w-full min-h-20 p-2 border rounded read-only:bg-gray-200" :readonly="editing" v-model="editData.anAdditionalComment"></textarea>
        </div>
